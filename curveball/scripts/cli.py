...
@cli.command()
def analyse(path, output_file, plate_folder, plate_file, blank_strain, ref_strain, param_min, max_time, guess, param_max, weights):
	...
	with click.progressbar(files, label='Processing files:', item_show_func=get_filename, color='green') as bar:
		for filepath in bar:
			file_results = _process_file(filepath, plate, blank_strain, ref_strain, max_time, guess, param_min, param_max, weights)
...
def _process_file(filepath, plate, blank_strain, ref_strain, max_time, guess, param_min, param_max, weights):
	...
	with click.progressbar(strains, label='Fitting strain growth curves', item_show_func=lambda s: s, color='green') as bar:
		for strain in bar:
			...
			_ = curveball.models.fit_model(strain_df, param_guess=guess, param_min=param_min, param_max=param_max, use_weights=weights, PLOT=PLOT, PRINT=VERBOSE)
			if PLOT:
